...
function formatNewsFeedsToOpml(feeds) {
...
<opml version="2.0">
  ...
  <body>
    ${feeds.map(feed => `<outline title="${escapeHtml(feed.extra.title || '')}" text="${escapeHtml(feed.extra.title || '')}" type="${feed.extra.feed_type || 'rss'}" xmlUrl="${escapeHtml(feed.feed_url)}" htmlUrl="${escapeHtml(feed.feed_url)}"/>`).join('\n    ')}
  </body>
</opml>`;
}
function parseOpmlFromTextContents(html) {
  const feedUrls = [];
...
export default function Feeds({
  ...
  const feeds = useSignal(initialFeeds);
  ...
  async function onClickDeleteFeed(feedId) {
  ...
  return h(Fragment, null, h("section", {
    ...
    type: "button"
  }, "Import OPML"), h("button", {
    class: `text-white block px-4 py-2 text-sm w-full text-left hover:bg-slate-600`,
    onClick: () => onClickExportOpml(),
    type: "button"
  }, "Export OPML")))), h("button", {
  ...
  }, newsFeed.extra.title || 'N/A'), h("div", {
    class: "text-slate-400"
  }, newsFeed.feed_url))), h("td", {
    class: "px-6 py-4 text-slate-200"
  }, newsFeed.last_crawled_at ? dateFormat.format(new Date(newsFeed.last_crawled_at)) : 'N/A'), h("td", {
    class: "px-6 py-4"
  }, h("div", {
    class: "text-xs font-semibold text-slate-200"
  }, newsFeed.extra.feed_type?.split('').map(character => character.toUpperCase()).join('') || 'N/A')), h("td", {
    class: "px-6 py-4"
  }, h("span", {
    ...
    width: 18,
    height: 18
  }), "Importing...") : null, !isDeleting.value && !isExporting.value && !isImporting.value ? h(Fragment, null, "\xA0") : null)));
}