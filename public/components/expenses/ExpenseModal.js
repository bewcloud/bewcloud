...
export default function ExpenseModal({
  ...
  const newExpenseCost = useSignal(expense?.cost ?? '');
  const newExpenseDescription = useSignal(expense?.description ?? '');
  const newExpenseBudget = useSignal(expense?.budget ?? 'Misc');
  const newExpenseDate = useSignal(expense?.date ?? '');
  ...
  return h(Fragment, null, h("section", {
  ...
  }, expense ? 'Edit Expense' : 'Create New Expense'), h("section", {
    ...
    onKeyDown: event => {
      if (event.key === 'Enter') {
        onClickSave(formatInputToNumber(newExpenseCost.value), newExpenseDescription.value, newExpenseBudget.value, newExpenseDate.value, newExpenseIsRecurring.value);
      }
    },
    onFocus: () => {
  ...
  }, sortedBudgetNames.map(budget => h("option", {
    value: budget,
    selected: newExpenseBudget.value === budget
  }, budget)))), h("fieldset", {
    class: "block mb-2"
  }, h("label", {
    class: "text-slate-300 block pb-1",
    ...
    class: "text-slate-300 block pb-1",
    for: "expense_is_recurring"
  }, "Is Recurring?"), h("input", {
    class: "input-field",
    type: "checkbox",
