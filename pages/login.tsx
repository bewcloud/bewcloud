...
async function get({ request, match, user, isRunningLocally }: RequestHandlerParams) {
  ...
  const htmlContent = renderToString(
    <Login
      ...
      isMultiFactorAuthEnabled={isMultiFactorAuthEnabled}
      isSingleSignOnEnabled={isSingleSignOnEnabled}
...
async function post({ request, match, user, isRunningLocally }: RequestHandlerParams) {
  if (user) {
    return new Response('Redirect', { status: 303, headers: { 'Location': `/` } });
  }
    ...
    <main>
      <section class='max-w-screen-md mx-auto flex flex-col items-center justify-center'>
        ...
        <form method='POST' class='mb-4'>
          ...
          ).map((field) => generateFieldHtml(field, formData || new FormData()))}
          ...
          {isMultiFactorAuthEnabled
            ? (
              <section class='mb-12 max-w-sm mx-auto'>
                ...
                {renderHydratableComponent(PasswordlessPasskeyLogin)}
              </section>
            )
            : null}
