import { PHOTO_IMAGE_EXTENSIONS, PHOTO_VIDEO_EXTENSIONS } from '/public/ts/utils/photos.ts';
export default function ListPhotos({
  files
}) {
  return h("section", {
    class: "mx-auto max-w-7xl my-8"
  }, files.length === 0 ? h("article", {
    class: "px-6 py-4 font-normal text-center w-full"
  }, h("div", {
    class: "font-medium text-slate-400 text-md"
  }, "No photos to show")) : h("section", {
    class: "w-full grid grid-cols-2 md:grid-cols-3 gap-4"
  }, files.map(file => {
    const lowercaseFileName = file.file_name.toLowerCase();
    const extensionName = lowercaseFileName.split('.').pop() || '';
    const isImage = PHOTO_IMAGE_EXTENSIONS.some(extension => extension === extensionName);
    const isVideo = PHOTO_VIDEO_EXTENSIONS.some(extension => extension === extensionName);
    return h("article", {
      class: "hover:opacity-70"
    }, h("a", {
      href: `/files/open/${encodeURIComponent(file.file_name)}?path=${encodeURIComponent(file.parent_path)}`,
      class: "flex items-center",
      target: "_blank",
      rel: "noopener noreferrer"
    }, isVideo ? h("video", {
      class: "h-auto max-w-full rounded-md",
      title: file.file_name
    }, h("source", {
      src: `/files/open/${encodeURIComponent(file.file_name)}?path=${encodeURIComponent(file.parent_path)}`,
      type: `video/${extensionName}`
    })) : null, isImage ? h("img", {
      src: `/photos/thumbnail/${encodeURIComponent(file.file_name)}?path=${encodeURIComponent(file.parent_path)}`,
      class: "h-auto max-w-full rounded-md",
      alt: file.file_name,
      title: file.file_name
    }) : null));
  })));
}